function [EstiLoc,LocErr,BS_sel] = gen_UElocation(sysPar,data,PE,AngleArray,RangeArray);
...
nBSsel = nBS;  %  the number of BS selected for positioning
...
for iUE = 1 : nUE
    ...
    for  ii = 1 : nn
        Z = reshape(ObAngle(:  ,( (segmSlot * (ii -1) +1): (segmSlot * ii) ) ), ...
            nBSsel * segmSlot, 1);%%读取估计角度
        pos_up = posReal_UE(:, segmSlot * (ii -1) +1) + randn(2, 1) *0.1;%%随机先生成一个位置pos_up进行迭代
        itemp = 0;
        while ( sum(( pos_up - pos_o) .^2) > 1e-3 && itemp < 30 )
            disp(sum(( pos_up - pos_o) .^2));
            pos_o = pos_up;
            t=segmSlot * (ii -1)+1;
            t2 = pos_BS{t};
            % 交换两列
            %data.Range_esti(:, [1, 2]) = data.Range_esti(:, [2, 1]);
            %r2= (RangeArray.^2)';
            r2 = sum( ( pos_o - t2 ) .^2, 1)';
            temp = ( pos_o - pos_BS{t})';%
            H = atan( temp(:, 2) ./ temp(:, 1) );
            H1 = repmat(H, segmSlot, 1);
            B11 = (pos_o - pos_BS{segmSlot * (ii -1)+1})' ./ r2;
            B13 = temp./ r2;
            B12 = [0 1 ;-1 0];
            B =  B11 *  B12;
...
%             pos_up = (B1'* B1)\ B1'* (Z - H1) + pos_o;
            ...
            B3 = B1';
            detaAng = Z - H1;
            pos_up = lsqminnorm(B2, B3) * detaAng;
            pos_up =  pos_up + pos_o;
            itemp= itemp +1;
        end
